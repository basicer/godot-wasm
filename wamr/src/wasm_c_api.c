...
#include "bh_log.h" // bh_log.h
#include "wasm_c_api_internal.h" // wasm_c_api_internal.h

#include "bh_assert.h" // bh_assert.h
#include "wasm_export.h" // wasm_export.h
#include "wasm_memory.h" // wasm_memory.h
#if WASM_ENABLE_INTERP != 0
#include "wasm_runtime.h" // wasm_runtime.h
#endif
...
wasm_memory_new_basic(wasm_store_t *store, const wasm_memorytype_t *type)
{
    ...
    memory->inst_late_bound =  malloc_internal(sizeof(WASMModuleInstanceCommon*));
...
wasm_memory_data(wasm_memory_t *memory)
{
    ...
    if (memory && memory->inst_late_bound && *memory->inst_late_bound && !memory->inst_comm_rt) {
        memory->inst_comm_rt = *memory->inst_late_bound;
    }
...
wasm_memory_grow(wasm_memory_t *memory, wasm_memory_pages_t delta)
{
    ...
    if (module_inst_comm->module_type == Wasm_Module_Bytecode) {
        ...
        WASMMemoryInstance *memory_inst =
            module_inst->memories[memory->memory_idx_rt];
        return wasm_memory_enlarge(memory_inst, delta);
    }
        ...
        const wasm_extern_vec_t *imports)
{
    ...
    for (i = 0, import_func_i = 0, import_global_i = 0; i < imports->num_elems;
        ...
        switch (wasm_extern_kind(import)) {
            ...
            case WASM_EXTERN_MEMORY:
            {
                wasm_memory_t* mem = wasm_extern_as_memory(import);
#if WASM_ENABLE_INTERP != 0 || true
                if (mem->inst_late_bound) {
                    *mem->inst_late_bound = (WASMModuleInstanceCommon*)module; 
                    break;
                }
#endif
                LOG_WARNING("link memory failed");
                goto failed;
            }
            case WASM_EXTERN_TABLE:
            {
                LOG_WARNING("doesn't support import tables for now, ignore them");
                break;
            }
            default:
            {
